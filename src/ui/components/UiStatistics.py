...
class Statistics(Gtk.Box):
    ...
    def __set_loaded_images(self, proj_with: str | None, proj_without: str | None):
    ...
    def __update_internal_ui(self, _):
        ...
        if UserRessources.project_images is None:
            self.__set_loaded_images(None, None)
        ...
        if UserRessources.project_images is not None and UserRessources.projects is not None:
            ...
            amt_with = len(list(filter(lambda proj: proj.get_raw_stand_number() in UserRessources.project_images, UserRessources.projects)))

            amt_without = len(list(filter(lambda proj: proj.get_raw_stand_number() not in UserRessources.project_images, UserRessources.projects)))

            self.__set_loaded_images(str(amt_with), str(amt_without))
